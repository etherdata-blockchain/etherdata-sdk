...
suspend fun Getdashrate(): String{
    ...
    val response: JsonRpcResponse<String> = client.post(url){
        ...
        body = JsonRpcRequest(method =  "miner_Getdashrate", params = listOf(), jsonrpc = "2.0", id = 1 )
    }
...
suspend fun setExtra(): Void{
    ...
    val response: JsonRpcResponse<Void> = client.post(url){
        contentType(ContentType.Application.Json)
        body = JsonRpcRequest(method =  "miner_setExtra", params = listOf(), jsonrpc = "2.0", id = 1 )
    }
    return response.result
...
suspend fun setGasPrice(price:Long): Void{
    ...
    val response: JsonRpcResponse<Void> = client.post(url){
        contentType(ContentType.Application.Json)
        body = JsonRpcRequest(method =  "miner_setGasPrice", params = listOf(price), jsonrpc = "2.0", id = 1 )
    }
    return response.result
...
suspend fun setEtherbase(etherbase:String): Void{
    ...
    val response: JsonRpcResponse<Void> = client.post(url){
        contentType(ContentType.Application.Json)
        body = JsonRpcRequest(method =  "miner_setEtherbase", params = listOf(etherbase), jsonrpc = "2.0", id = 1 )
    }
    return response.result
