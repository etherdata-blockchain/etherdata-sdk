...
data class NodeInfoResponseNodeInfo (val enode:String,val id:String,val ip:String,val listenAddr:String,val ports:NodeInfoResponsePorts)
...
data class PeersArray (val caps:List<String>,val id:String,val name:String,val network:Network,val protocols:List<Protocols>)
...
suspend fun startRPC(host:String, port:Long, cors:String, apis:String): Boolean{
    ...
    val response: JsonRpcResponse<Boolean> = client.post(url){
        contentType(ContentType.Application.Json)
        body = JsonRpcRequest(method =  "admin_startRPC", params = listOf(host, port, cors, apis), jsonrpc = "2.0", id = 1 )
    }
    return response.result
