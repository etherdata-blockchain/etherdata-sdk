...
data class Accounts (val balance:String,val code:String,val codeHash:String,val nonce:Long,val root:String,val storage:List<Any>)
...
data class StrucrtLogs (val depth:Long,val error:String,val gas:Long,val gasCost:Long,val memory:List<String>,val op:String,val pc:Long,val stackArray:List<String>,val storage:Storage)

data class TraceBlockResponseBlcok (val gas:Long,val returnValue:String,val strucrtLogs:List<StrucrtLogs>)

data class TraceTransactionResponseTransaction (val gas:Long,val returnValue:String,val strucrtLogs:List<StrucrtLogs>)
...
suspend fun backtraceAt(locationJavascript_based:Any): Void{
    ...
    val response: JsonRpcResponse<Void> = client.post(url){
        ...
        body = JsonRpcRequest(method =  "debug_backtraceAt", params = listOf(locationJavascript_based), jsonrpc = "2.0", id = 1 )
    }
...
suspend fun traceBlockByNumber(blockNum:String): Any{
    ...
    val response: JsonRpcResponse<Any> = client.post(url){
        contentType(ContentType.Application.Json)
        body = JsonRpcRequest(method =  "debug_traceBlockByNumber", params = listOf(blockNum), jsonrpc = "2.0", id = 1 )
    }
    return response.result
...
suspend fun traceBlockByHash(blockHash:String): Any{
    ...
    val response: JsonRpcResponse<Any> = client.post(url){
        contentType(ContentType.Application.Json)
        body = JsonRpcRequest(method =  "debug_traceBlockByHash", params = listOf(blockHash), jsonrpc = "2.0", id = 1 )
    }
    return response.result
...
suspend fun standardTraceBlockToFile(block:String, txHash:String, disableMemory:Boolean): List<String>{
    ...
    val response: JsonRpcResponse<List<String>> = client.post(url){
        contentType(ContentType.Application.Json)
        body = JsonRpcRequest(method =  "debug_standardTraceBlockToFile", params = listOf(block, txHash, disableMemory), jsonrpc = "2.0", id = 1 )
    }
    return response.result
...
suspend fun standardTraceBadBlockToFile(): Void{
    ...
    val response: JsonRpcResponse<Void> = client.post(url){
        contentType(ContentType.Application.Json)
        body = JsonRpcRequest(method =  "debug_standardTraceBadBlockToFile", params = listOf(), jsonrpc = "2.0", id = 1 )
    }
    return response.result
...
*/
suspend fun traceTransaction(hash:String, disableStorage:Boolean, disableMemory:Boolean, disableStack:Boolean, tracer:String, timeout:String): TraceTransactionResponseTransaction{
    
    val response: JsonRpcResponse<TraceTransactionResponseTransaction> = client.post(url){
        contentType(ContentType.Application.Json)
        body = JsonRpcRequest(method =  "debug_traceTransaction", params = listOf(hash, disableStorage, disableMemory, disableStack, tracer, timeout), jsonrpc = "2.0", id = 1 )
    }
    return response.result
...
suspend fun traceCall(to:String, from:String, gas:String, gasPrice:Long, value:Long, data:String): Any{
    ...
    val response: JsonRpcResponse<Any> = client.post(url){
        contentType(ContentType.Application.Json)
        body = JsonRpcRequest(method =  "debug_traceCall", params = listOf(to, from, gas, gasPrice, value, data), jsonrpc = "2.0", id = 1 )
    }
    return response.result
...
suspend fun vmodule(messageRestrictions:String): String{
    ...
    val response: JsonRpcResponse<String> = client.post(url){
        contentType(ContentType.Application.Json)
        body = JsonRpcRequest(method =  "debug_vmodule", params = listOf(messageRestrictions), jsonrpc = "2.0", id = 1 )
    }
    return response.result
